...
```
Generator(seed: WorldSeed + Index, sequence: TypeSequence)
```

1.  **World Seed:** The global 64-bit integer provided by the developer (e.g., `42`).
2.  **Index:** The position in the array (e.g., `50`).
3.  **Type Sequence:** A unique constant ID for each data type (e.g., `101` for Users, `105` for Addresses) that serves as the stream identifier to prevent correlation between different arrays.
