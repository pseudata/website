...
* **Connector (Low bits, bits 0-2):** Configurable width (**default: 3 bits**). This is the "slot" that gets modified to navigate between related indices. With 3 bits, you get 8 possible relationships per neighborhood.
* **Neighborhood (Middle bits, bits 3-22):** Remaining bits (**default: 20 bits** when using 17 island + 3 connector). Indices sharing the same neighborhood and island bits are considered related.
* **Island (High bits, bits 23-39):** Configurable width (**default: 17 bits**). These bits determine sharding - they remain unchanged during relationship calculations, ensuring related indices always hash to the same partition.
