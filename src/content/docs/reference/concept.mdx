...
3.  **Type Sequence:** A unique constant ID for each data type (e.g., `101` for Users, `110` for Addresses) that serves as the stream identifier to prevent correlation between different arrays. The sequence value is internally transformed to an odd increment (`inc = (seq << 1) | 1`) as required by PCG32's LCG foundation.
