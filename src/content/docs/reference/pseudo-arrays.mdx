---
title: Pseudo-Arrays
description: Infinite deterministic arrays that generate elements on-demand using pseudo-random algorithms, providing O(log n) access with zero memory overhead.
sidebar:
  order: 4
head:
  - tag: script
    ...
    content: |
      {
        ...
        "mainEntityOfPage": {
          "@type": "WebPage",
          "@id": "https://pseudata.dev/reference/pseudo-arrays/"
        },
        "keywords": ["pseudo-array", "infinite array", "deterministic", "O(log n)", "zero memory", "pseudo-random", "PCG32", "virtual array"]
      }
---

import { Tabs, TabItem, Aside } from '@astrojs/starlight/components';

A *pseudo-array* is an infinite, deterministic array that generates elements on-demand using pseudo-random algorithms. Each *index* always produces the same element for a given *world-seed* and *type-sequence*, with **$O(\log n)$ access** and **zero memory overhead**.
...
* **$O(\log n)$ access**: Direct index access without iteration
...
billionthUser := users.At(1_000_000_000)  // O(log n)
```
  </TabItem>
  <TabItem label="Java">
```java
...
// Access any index instantly
User billionthUser = users.at(1_000_000_000);  // O(log n)
```
  </TabItem>
...
# Access any index instantly
billionth_user = users[1_000_000_000]  # O(log n)
```
  </TabItem>
...
// Access any index instantly
const billionthUser = users.at(1_000_000_000);  // O(log n) - no iteration needed
```
  </TabItem>
</Tabs>
...
Access any index directly with $O(\log n)$ complexity:
...
Every element generated by *pseudo-arrays* has a [Pseudo ID](/reference/pseudo-ids/) that encodes its position:
...
You can use *pseudo links* with the `PseudoLink` class ([see reference](/reference/pseudo-links/)) to create indices with relational properties:
...
- **[Pseudo-IDs](/reference/pseudo-ids/)** - Learn about the deterministic IDs that identify each element
- **[Pseudo-Links](/reference/pseudo-links/)** - Use coordinate-based indices for relational test data
