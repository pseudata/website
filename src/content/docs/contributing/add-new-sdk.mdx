...
function emitRustInterface(interfaceName, methods) {
    const methodDecls = methods.map(m => {
        const params = m.parameters
            .map(p => `${snakeCase(p.name)}: ${mapRustType(p.type)}`)
            .join(', ');
...
async function emitAllInterfaces(program, interfaces) {
    ...
    // Add Rust
    const rustCode = interfaces.map(i => 
        emitRustInterface(i.name, i.methods)
    ).join('\n\n');
    
    await program.host.writeFile(
        "sdks/rust/src/primitives.rs",
        rustCode
    );
}
```
...
### 7.3: Update Reusable CI Workflow

Add your language setup to `.github/workflows/reusable-ci.yml`:
...
# Or for C#:
- name: Setup .NET
  if: inputs.sdk == 'csharp'
  uses: actions/setup-dotnet@v4
  with:
    dotnet-version: '9.0'
    cache: true
    cache-dependency-path: '${{ inputs.path }}/YourProject.csproj'
```

**Note**: Use the appropriate GitHub Action for your language and ensure all actions are pinned to commit SHAs (not tags) for security:
```yaml
uses: actions-rust-lang/setup-rust-toolchain@1fbea72663f6d4c03efaab13560c8a24cfd2a7cc # v1.0.0
```

### 7.4: Create CI Workflow
...
on:
  push:
    branches: [ "main" ]
    paths: ['sdks/rust/**', 'Taskfile.yml', '.github/workflows/ci-rust.yml', '.github/workflows/reusable-ci.yml']
  pull_request:
    branches: [ "main" ]
    paths: ['sdks/rust/**', 'Taskfile.yml', '.github/workflows/ci-rust.yml', '.github/workflows/reusable-ci.yml']
...
**Publishing Examples by Language**:

<details>
<summary>C# → NuGet</summary>
...
<details>
<summary>PHP → Packagist</summary>

```yaml
# PHP uses Composer and Packagist
# Packagist automatically pulls from GitHub releases
# No explicit publish job needed - just ensure composer.json has correct version

# Optional: Add webhook trigger in Packagist settings
```

**Setup**: Configure Packagist webhook for auto-updates
</details>

<details>
<summary>Ruby → RubyGems</summary>
  ...
  steps:
    ...
    - name: Publish
      ...
      run: |
        task ruby:setup
        cd sdks/ruby
        gem build pseudata.gemspec
        gem push pseudata-*.gem
```

**Required Secret**: `RUBYGEMS_API_KEY`
</details>

### 7.6: Configure Repository Secrets
...
- Rust: `CARGO_REGISTRY_TOKEN` (from crates.io)
- C#: `NUGET_API_KEY` (from nuget.org)
- Ruby: `RUBYGEMS_API_KEY` (from rubygems.org)
- PHP: No secret needed (Packagist auto-syncs)

### 7.7: Test CI Workflow

1. **Push to feature branch**:
   ```bash
   git checkout -b test/add-rust-sdk
   git add sdks/rust/
   git commit -m "feat: add Rust SDK"
...
4. **Test release workflow** (staging):
   - Create test release on staging branch
   - Verify publishing job runs correctly
...
1. **Root README.md**: Add to language list
2. **sdks/README.md**: Add SDK table entry
3. **Website**: Update language support documentation
4. **Changelog**: Document new SDK in release notes
...
- [ ] **Reusable CI**: Added language setup step to `.github/workflows/reusable-ci.yml`
- [ ] **CI Workflow**: Created `.github/workflows/ci-yourlanguage.yml`
- [ ] **Release Workflow**: Added publish job to `.github/workflows/release.yml`
- [ ] **Secrets**: Configured package registry API tokens in GitHub settings
- [ ] **Testing**: Verified CI runs on PRs and pushes to main
