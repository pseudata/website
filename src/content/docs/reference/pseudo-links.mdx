    ...
    content: |
      {
        ...
        "mainEntityOfPage": {
          "@type": "WebPage",
          "@id": "https://pseudata.dev/reference/pseudo-links/"
        },
        "keywords": ["PseudoLink", "pseudo link", "coordinate system", "stateless relationships", "bit manipulation", "O(1)", "deterministic", "sharding"]
      }
---
...
The key insight: by treating an index as structured coordinates (neighborhood, connector, island) rather than a scalar value, you can instantly navigate between related indices through simple bitwise operations. The indices can point to items in the same PseudoArray (same object type) or different arrays (like users and groups). When working with [Pseudo-IDs](/reference/pseudo-ids/), a *pseudo-link* operates on the 40-bit *index* component, while the *world-seed* and *type-sequence* remain unchanged.
...
* **Island (Low bits):** Configurable width (**default: 17 bits**). These bits determine sharding - they remain unchanged during relationship calculations, ensuring related indices always hash to the same partition.
* **Connector (Middle bits):** Configurable width (**default: 3 bits**). This is the "slot" that gets modified to navigate between related indices. With 3 bits, you get 8 possible relationships per neighborhood.
* **Neighborhood (High bits):** Remaining bits (**default: 20 bits** when using 17 island + 3 connector). Indices sharing the same neighborhood and island bits are considered related.

**The bit layout is configurable.** You choose `islandBits` and `connectorBits` in the constructor. The remaining bits become the neighborhood. Choose your layout based on your requirements: more connector bits = more relationships per neighborhood, more island bits = finer sharding granularity.

**Why this bit order?** Island occupies the low bits to enable simple modulo operations for sharding ($index \bmod 2^{islandBits}$). Connector sits in the middle for clean bit manipulation without affecting the island. Neighborhood uses the high bits, following the common pattern of hierarchical grouping where high-order bits represent broader categorization.
...
declare class PseudoLink {
     ...
     * @param connectorBits - Number of bits for relationship slots (default: 3).
     * The remaining bits (40 - islandBits - connectorBits) become the neighborhood.
     * Choose your layout based on requirements: more connectorBits = more relationships.
     */
    constructor(islandBits: number, connectorBits: number);
...
class PseudoLink {
    ...
    constructor(islandBits: number, connectorBits: number) {
        ...
        this.connectorBits = BigInt(connectorBits);
        this.neighborhoodBits = 40n - this.islandBits - this.connectorBits;
        
        this.shift = this.islandBits;
    ...
    public encode(island: number, neighborhood: number, connector: number): bigint {
        ...
        const islandPart = BigInt(island) & this.islandMask;
        const connectorPart = (BigInt(connector) & this.connectorMask) << this.shift;
        const neighborhoodPart = (BigInt(neighborhood) & this.neighborhoodMask) << (this.shift + this.connectorBits);
        
        return neighborhoodPart | connectorPart | islandPart;
    }
    ...
    public decode(index: bigint | number): LinkComponents {
        ...
        const island = Number(bigIndex & this.islandMask);
        const connector = Number((bigIndex >> this.shift) & this.connectorMask);
        const neighborhood = Number((bigIndex >> (this.shift + this.connectorBits)) & this.neighborhoodMask);
    ...
    public resolve(sourceIndex: bigint | number, connector: number): bigint {
        ...
        const clearedBase = bigIndex & ~(this.connectorMask << this.shift);
        return clearedBase | (BigInt(connector) << this.shift);
    }
...
**Cross-Neighborhood Controlled Pattern**

![Same-Entity Relationship Diagram](/images/relation-same-entity.svg)

Entities in different *neighborhoods* but the same *island* create explicit, controlled relationships similar to foreign keys. This avoids the automatic many-to-many coupling of within-neighborhood patterns.

**Coordinate Structure:**
- Fixed: *island* (e.g., `1`)
- Variable: *neighborhood* (e.g., User at N:`1000`, Mentor at N:`2000`)
- *Connector* can be fixed or variable depending on use case

**Characteristics:**
- **Cardinality**: Controlled and explicit (not automatic)
- **Navigation**: Construct indices for both directions using `encode()`
- **Shard Safety**: Maintained through same island value
- **Use Case**: Hierarchical or explicit relationships (Mentee→Mentor, Employee→Manager)

**Pattern Example**: Mentorship
- Mentees occupy neighborhoods 1000-1999
- Mentors occupy neighborhoods 2000-2999
- Alice at (I:1, N:1000, C:0) → Bob at (I:1, N:2000, C:0)
- Relationship is explicit, not derived from automatic connector iteration

**Advantage**: Models real-world relationships where connections are explicit assignments, not automatic groupings. More flexible than within-neighborhood patterns but requires coordinate convention planning.
...
Let an index $Idx$ be defined as the concatenation of three bit-segments: Neighborhood ($N$), Connector ($C$), and Island ($I$).
...
### Symmetry Proof
Since $N$ and $I$ are preserved during the transformation, any two indices $Idx_1$ and $Idx_2$ are part of the same *pseudo-link* cell if and only if their Neighborhood and Island segments match:
...
### Sharding Invariance
Distributed systems often shard data using a modulo operation on the ID. In a *pseudo-link*, given $islandBits = 17$:

$ShardID = Idx \bmod 2^{17}$

Because the resolution function $f(Idx, k)$ only modifies bits at positions $\ge 17$, the lower 17 bits are invariant:

$ShardID(Idx) \equiv ShardID(f(Idx, k))$
...
A *pseudo-link* operates on the **40-bit index component** of [Pseudo-IDs](/reference/pseudo-ids/). When you create a PseudoID with `encodeId(worldSeed, typeSeq, index)`, the `index` parameter is just a 40-bit number. `PseudoLink` provides a coordinate system for manipulating those bits to create relationships.
...
- **[Pseudo-IDs](/reference/pseudo-ids/)** - Learn about the complete *pseudo-id* structure and how the 40-bit index component works with *pseudo-links*
- **[Pseudo-Arrays](/reference/pseudo-arrays/)** - Understand how *pseudo-arrays* use indices to provide $O(\log n)$ access to deterministic mock data
