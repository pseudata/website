...
Pseudata supports **17 locales** across multiple languages and regions, providing culturally appropriate names, addresses, and geographic data. By default, Pseudata loads only the **US locale** (`en_US`). For multi-region support, you can import **regional bundles** that group related locales.
...
Bundles are organized into tiers based on geographic, business, and cultural groupings.
...
**APAC (Asia-Pacific)**
- Locales: `ja_JP`, `zh_CN`, `hi_IN`, `vi_VN`
...
Pseudata currently supports 17 locales across 12 languages and 16 countries. Each locale includes culturally appropriate data for names, addresses, and geographic information.
...
- Language data duplicated for every locale (months, weekdays, word lists × 17 locales = 17 copies)
